...
/**
 * Keeps every terminal tab named after the directory the shell is currently sitting in, so a `cd`
 * is reflected in the tab without any action from the user. Off by default, enabled by
...
class CurrentDirectoryTabNameTracker(
    ...
    private fun applyNames(
        ...
        for (content in contents) {
            ...
            // Nothing to follow: the tab keeps the name it was opened under. resolveFromEngine has
            // already logged why.
            val directoryName = directories[content]
                ?.let { TerminalTabsUtil.directoryNameOf(it, settings.currentDirectoryParents) }
                ?: continue
            ...
            val newDisplayName = TerminalTabsUtil.incrementNumberInName(
                ...
                TerminalTabsUtil.appendDate(directoryName, settings)
            )
